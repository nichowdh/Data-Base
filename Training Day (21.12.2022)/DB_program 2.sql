...
select min(Esalary) from Future_Employee where dept ='ECE';
select min(Esalary) from Future_Employee;
# subQuery
select Ename,dept,Esalary from Future_Employee where Esalary=(select min(Esalary) from Future_Employee);
# max
select dept,max(Esalary) as ' max_dpt_salary' from Future_Employee group by dept;
select max(Esalary) from Future_Employee where dept = 'ECE';
select max(Esalary) from Future_Employee;
# subQuery
select Ename,dept,Esalary from Future_Employee where Esalary=(select max(Esalary) from Future_Employee);
# having
select dept,sum(working_hour) as 'total_hours' from Future_Employee group by 
dept having sum(working_hour);
